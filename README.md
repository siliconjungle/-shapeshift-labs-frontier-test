...
```sh
npm install @shapeshift-labs/frontier-test
```
...
const tests = compileTestManifest(createTestManifest({
  id: 'app.tests',
  specs: [{
    id: 'spec.todos.complete',
    feature: 'todos',
    given: { stateFixture: 'todos.open' },
    ...
    commands: ['cmd.todos.unit']
  }],
  commands: [{ id: 'cmd.todos.unit', command: 'npm', args: ['run', 'test:todos'] }]
}));

const impact = traceTestImpact(tests, {
...
Run the comparison/control benchmark:

```sh
npm run bench:competitors
```

The package benchmark writes `benchmarks/results/frontier-test-package-bench-latest.json` when run from the monorepo. The competitor/control benchmark writes `benchmarks/results/frontier-test-competitors-latest.json`. Frontier-only package measurements cover manifest creation, compilation, validation, coverage queries, impact tracing, run planning, run recording, report adapters, JSONL, and proof hashing.
